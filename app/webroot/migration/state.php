...
$sql = "select  RID, [StateTitle],[ModifiedDate] from [HCM_STATES] order by RID asc";
$result = mssql_query($sql);

while($row = mssql_fetch_assoc($result)){
	$id = $row['RID'];
	$state = $row['StateTitle'];	
	$modified = date('Y-m-d H:i:s', strtotime($row['ModifiedDate']));
	
	
	$save_sql = "insert into state (id, state,created_date)values('$id', '$state','$modified')";	
