...
if (!$link) {
	...
	$sql = "insert into hc_sync (module,status,sync_time,error_msg) values('5',	'0','$time','Unable to Connect to HC Server')";
...
// get last sync time
$sql = "select sync_time from hc_sync where module = '5'  and status = '1' order by id desc limit 1";
$result = mysql_query($sql);
$row = mysql_fetch_assoc($result);
$sync_time =  date('Y-m-d H:i', strtotime($row['sync_time']) - 19800);
$sql = "select RID,[ReqID], [AnchorID], [TeamUserID], [FromDate], [ToDate] from [HC_REQ_TEAM] 
where (FromDate >= '$sync_time' or ToDate >= '$sync_time') order by RID asc";
...
while($row = mssql_fetch_assoc($result)){
	...
	$from = date('Y-m-d H:i:s', strtotime($row['FromDate'])  + 19800);
	$to = date('Y-m-d H:i:s', strtotime($row['ToDate'])  + 19800);
	
	// if exists in mysql database, insert or update
	$sql = "select id from req_team where id = '$id'";
	...
	$row_id = $row_id['id'];
	if($row_id != ''){		
		$save_sql = "update req_team set id='$id', requirements_id='$req_id', users_id='$user_id', created_by='$created_by',
		from_date='$from',to_date='$to' where id = '$row_id'";			
...
// update  sync status
$time = date('Y-m-d H:i:s');
$sql = "insert into hc_sync (module,status,sync_time,no_update,no_insert,fail_update,fail_insert) values('5',
