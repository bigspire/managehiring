...
if (!$link) {
	...
	$sql = "insert into hc_sync (module,status,sync_time,error_msg) values('11','0','$time','Unable to Connect to HC Server')";
...
// get last sync time
$sql = "select sync_time from hc_sync where module = '11' and status = '1' order by id desc limit 1";
$result = mysql_query($sql);
$row = mysql_fetch_assoc($result);
$sync_time =  date('Y-m-d H:i', strtotime($row['sync_time']) - 19800);
$sql = "select RID, [JobTitle],ModifiedDate from [HCM_JOBTITLE]  where ModifiedDate >= '$sync_time' order by RID asc";
...
while($row = mssql_fetch_assoc($result)){
	...
	$sql = "select id from designation where id = '$id'";
	...
	$row_id = $row_id['id'];
	if($row_id != ''){		
		$save_sql = "update designation set id='$id', designation='$desig',created_date='$modified' where id = '$row_id'";			
...
// update  sync status
$time = date('Y-m-d H:i:s');
$sql = "insert into hc_sync (module,status,sync_time,no_update,no_insert,fail_update,fail_insert) values('11',
