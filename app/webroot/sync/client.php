...
if (!$link) {
	...
	$sql = "insert into hc_sync (module,status,sync_time,error_msg) values('8',	'0','$time','Unable to Connect to HC Server')";
...
// get last sync time
$sql = "select sync_time from hc_sync where module = '8'  and status = '1' order by id desc limit 1";
$result = mysql_query($sql);
...
from HC_CLIENTS  where (CreatedDate >= '$sync_time' or ModifiedDate >= '$sync_time')   order by RID asc";
...
while($row = mssql_fetch_assoc($result)){
	...
	$row_id = $row_id['id'];
	if($row_id != ''){		
		$save_sql = "update clients set id='$id', client_name='$client_name',phone='$phone', address='$address',
		res_location_id='$loc', created_date= '$created',created_by='$created_by',modified_date='$modified',
		modified_by='$modified_by',status='$status' where id = '$row_id'";			
...
// update  sync status
$time = date('Y-m-d H:i:s');
$sql = "insert into hc_sync (module,status,sync_time,no_update,no_insert,fail_update,fail_insert) values('8',
