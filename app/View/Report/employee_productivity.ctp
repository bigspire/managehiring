...
<?php echo $this->Form->input('from', array('div'=> false,'type' => 'text', 'label' => false, 'class' => 'input-small input-monthrange', 'required' => false, 'placeholder' => 'mm/yyyy', 'autocomplete' => 'off', 'value' => $FINSTART)); ?> 
						...
						<?php echo $this->Form->input('to', array('div'=> false,'type' => 'text', 'label' => false, 'class' => 'input-small input-monthrange', 'required' => false, 'placeholder' => 'mm/yyyy', 'autocomplete' => 'off', 'value' => $FINEND)); ?> 
...
<input type="hidden" name="end_date" id="end_date" value="<?php echo $end_date;?>"/>	
				...
				<div class="span12">	
						...
						<h3 class="heading" style="margin-bottom:0;border:;clear:left;">Employee Productivity (Table View)
							...
							<table class="table table-hover table-bordered table-striped printAreaTable" style="margin: 15px 0px;">
								<thead>
									...
									<tr>
										...
										<th width="100" style="min-width: 0px; max-width: none;"><a href="#">Employee</a></th>										
										
										<?php foreach($monthDetail as $month):?>
										<th width="80" style="min-width: 0px; max-width: none;text-align:center"><a rel="tooltip" title="" href="#"><?php echo $month;?></a></th>
										<?php endforeach;?>
		...
		<div class="span12">
						<h3 class="heading" style="margin-bottom:0;border:;clear:left;">Employee Productivity (Graph View)
  ...
  <script type="text/javascript">
    ...
    function drawChart() {
      var data = google.visualization.arrayToDataTable([
		...
		<?php $i = 0; 
		foreach($monthDetail as $id => $month): ?>
		<?php //if($cv_sent_shortlist[$i] != '' || $cv_sent_interview[$i] != '' || $cv_sent_offer[$i] != '' || $cv_sent_join[$i] != '' || $cv_sent_billed[$i] != '' || 	$opening_work_billed[$i] != ''):?>
        ['<?php echo ucwords($month);?>', <?php echo $cv_sent_shortlist[$i];?>, <?php echo $cv_sent_interview[$i];?>, <?php echo $cv_sent_offer[$i];?>, <?php echo $cv_sent_join[$i];?>, <?php echo $cv_sent_billed[$i];?>,<?php echo $opening_work_billed[$i];?>],   
