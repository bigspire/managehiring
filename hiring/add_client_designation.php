...
if(!empty($_POST)){	
	if($_GET['action'] == 'dropdown'){
		...
		$fieldtype = array('0');
		$actualfield = array('client designation ');
	...
	$query = "CALL check_designation_exist('0', '".$fun->is_white_space($_POST['designation'])."','CL')";
	...
	if(empty($test)){
		if($row['total'] == '0'){
			...
			$query = "CALL add_designation('".$_SESSION['user_id']."','CL',
			...
			try{
				// calling mysql exe_query function
				if(!$result = $mysql->execute_query($query)){
					throw new Exception('Problem in executing add client designation');
				}
				$row = $mysql->display_result($result);
				...
				if(!empty($last_id)){
					if(empty($action)){
						...
						header('Location: client_designation.php?status=created');		
					}else{
						$smarty->assign('form_sent' , 1);
						$msg = "Client Designation added successfully";
...
// assign page title
$smarty->assign('page_title' , 'Add Client Designation - Manage Hiring');  
// assigning active class status to smarty menu.tpl
$smarty->assign('setting_active','active');
// display smarty file
$smarty->display('add_client_designation.tpl');
?>