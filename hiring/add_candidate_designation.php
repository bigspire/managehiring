<?php
/* 
Purpose : To add candidate designation
Created : Nikitasa
Date : 02-06-2018
*/
...
if(!empty($_POST)){	
	if($_GET['action'] == 'dropdown'){
		...
		$fieldtype = array('0');
		$actualfield = array('candidate designation ');
	...
	$query = "CALL check_designation_exist('0', '".$fun->is_white_space($_POST['designation'])."','CA')";
	...
	if(empty($test)){
		if($row['total'] == '0'){
			...
			$query = "CALL add_designation('".$_SESSION['user_id']."','CA',
			...
			try{
				// calling mysql exe_query function
				if(!$result = $mysql->execute_query($query)){
					throw new Exception('Problem in executing add candidate designation');
				}
				$row = $mysql->display_result($result);
				...
				if(!empty($last_id)){
					if(empty($action)){
						...
						header('Location: candidate_designation.php?status=created');		
					}else{
						$smarty->assign('form_sent' , 1);
						$msg = "Candidate Designation added successfully";
...
// assign page title
$smarty->assign('page_title' , 'Add Candidate Designation - Manage Hiring');  
// assigning active class status to smarty menu.tpl
$smarty->assign('setting_active','active');
// display smarty file
$smarty->display('add_candidate_designation.tpl');
?>