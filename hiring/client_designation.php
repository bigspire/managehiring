<?php
/* 
   Purpose : To list and search client designation.
	Created : Nikitasa
	Date : 02-06-2018
*/
...
$query = "CALL list_designation('".$keyword."','CL','".$status."','0','0','','','".$_GET['action']."')";
try{
	if(!$result = $mysql->execute_query($query)){
		throw new Exception('Problem in executing list client designation page');
	}
...
// fetch all records
$query =  "CALL list_designation('".$keyword."','CL','".$status."','$start','$limit','".$field."','".$order."','".$_GET['action']."')";
try{
	if(!$result = $mysql->execute_query($query)){
		throw new Exception('Problem in executing list client designation page');
	}
	...
	if($_GET['action'] == 'export'){ 
      ...
      $excelObj->printHeader($header = array('Client Designation','Status','Created Date','Modified Date') ,$col = array('A','B','C','D'));  
		// function to print the excel data
		$excelObj->printCell($data, $count,$col = array('A','B','C','D'), $field = array('designation','status','created_date','modified_date'),'Client Designation_'.$current_date);
	}	
	
	// create,update,delete message validation
	if($_GET['status'] == 'deleted' || $_GET['status'] == 'created' || $_GET['status'] == 'updated'){
 	 $success_msg = 'Client Designation ' . ucfirst($_GET['status']) . ' Successfully';
...
// assign page title
$smarty->assign('page_title' , 'Client Designation - Manage Hiring');  
// assigning active class status to smarty menu.tpl
$smarty->assign('setting_active','active');
// display smarty file
$smarty->display('client_designation.tpl');
?>